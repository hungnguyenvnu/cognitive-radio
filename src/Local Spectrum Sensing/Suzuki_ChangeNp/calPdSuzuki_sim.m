...
for ii = 1 : length(Pf)
    noise = randn(NS, 2*m);   % Gaussian noise with mean 0 and variance 1
    ...
    threshold_sim(ii) = energy_descend(ceil(Pf(ii)*NS));
end

% Simulated probability of detection for Suzuki channel
Pd = [];
for ii = 1:length(Pf)
    s = randi([0 1], NS, 2*m);        % Generating 0 and 1 for BPSK
    signal = (2.*s - 1);                  % BPSK modulation
    
    % Lognormal
    n1 = sqrt(var_gauss)*randn(NS, 1) + mean_gauss;
    l1 = exp(n1 + eps);
    % Rayleigh
    x1 = sqrt(variance) * randn(NS, 1);
    x2 = sqrt(variance) * randn(NS, 1);
    r1 = sqrt((x1).^2 + (x2).^2);
    r2 = r1 ./ norm(r1);
    ...
    count = sum(level);
    Pd = [Pd count/NS];     % The probability of detection
end
